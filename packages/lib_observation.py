...
def observation_mola(only_location=None):
    ...
    path = f'{data_dir}/obs_mola/MOLA_cloudaltis_1x1.nc'
...
def observation_tes(target, year=None):
    ...
    directory_tes = f'{data_dir}/obs_tes/'
...
def observation_pfs(target, year=None):
    ...
    # 1-D
    data_lat = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target='lat')  # Latitude
    data_lon = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target='lon')  # Longitude
    data_pressure = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target='Pressures')  # Pressure
    ...
    if target in ['loct', 'T_surf', 'p_surf', 'h_surf', 'dust', 'dust_norm', 'ice']:
        ...
        data = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target=target)
    ...
    if year:
        ...
        data_ls = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target='ls_MY')  # Ls incremented
        data_ls = masked_outside(data_ls, 360 * iyear, 360 * (iyear + 1))
    else:
        data_ls = get_data(f'{data_dir}/obs_pfs/PFS_dataset_20793.nc', target='ls')  # Ls
...
def mesospheric_clouds_observed():
    ...
    directory = f'{data_dir}/obs_mesoco2cloud/'
...
def mesospheric_clouds_altitude_localtime_observed(instrument):
    ...
    data_ls, data_lat, data_lon, data_lt, data_alt, data_alt_min, data_alt_max = None, None, None, None, None, None, \
                                                                                 None
    folder = f'{data_dir}/obs_mesoco2cloud_alti/'
    if instrument == 'HRSC':
...
def simulation_mvals(target, localtime):
    ...
    if target in ['ps', 'tsurf', 'Time', 'latitude', 'longitude', 'altitude']:
        if localtime == 2:
            filename = f'{data_dir}/simu_mvals/concat_vars_3D_LT_2h_Ls.nc'
        if localtime == 14:
            filename = f'{data_dir}/simu_mvals/concat_vars_3D_LT_14h_Ls.nc'
    elif target in ['temp']:  # TODO: MISSING THIS FILE
        filename = f'{data_dir}/simu_mvals/concat_vars_4D_P_LT_14h_Ls.nc'
...
def viking_lander(lander, mcd):
    ...
    if mcd:
        if lander == 1:
            path = f'{data_dir}/obs_viking_landers/viking_lander1_pression_mcd.dat'
        elif lander == 2:
            path = f'{data_dir}/obs_viking_landers/viking_lander2_pression_mcd.dat'
        else:
        ...
        sols_0 = 0
        if lander == 1:
            path = f'{data_dir}/obs_viking_landers/viking_lander1_pression.dat'
            sols_0 = 209  # http://www-mars.lmd.jussieu.fr/mars/time/martian_time.html , ls=97°
        elif lander == 2:
            path = f'{data_dir}/obs_viking_landers/viking_lander2_pression.dat'
...
def get_polar_cap():
    filename = f'{data_dir}/obs_titus/co2icecover_titus.nc'
